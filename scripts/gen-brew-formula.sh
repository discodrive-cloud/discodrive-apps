#!/bin/sh
# Emit the Homebrew formula for the released daemon tarballs of a given version.
# usage: gen-brew-formula.sh <version>   (e.g. 0.0.3) → formula on stdout
# Downloads the four published tarballs to hash them, so the release assets
# must already exist on GitHub.
set -eu

VER="${1:?usage: gen-brew-formula.sh <version>}"
BASE="https://github.com/kosmosoid/discodrive-apps/releases/download/v${VER}"

sha() {
  if command -v sha256sum >/dev/null 2>&1; then
    curl -fsSL "$BASE/discodrive-daemon-$1.tar.gz" | sha256sum | cut -d' ' -f1
  else
    curl -fsSL "$BASE/discodrive-daemon-$1.tar.gz" | shasum -a 256 | cut -d' ' -f1
  fi
}

SHA_DARWIN_ARM64=$(sha darwin-arm64)
SHA_DARWIN_AMD64=$(sha darwin-amd64)
SHA_LINUX_ARM64=$(sha linux-arm64)
SHA_LINUX_AMD64=$(sha linux-amd64)

cat <<EOF
class DiscodriveDaemon < Formula
  desc "Headless sync daemon for the DiscoDrive personal cloud"
  homepage "https://github.com/kosmosoid/discodrive-apps"
  version "${VER}"
  license "PolyForm-Noncommercial-1.0.0"

  on_macos do
    on_arm do
      url "${BASE}/discodrive-daemon-darwin-arm64.tar.gz"
      sha256 "${SHA_DARWIN_ARM64}"
    end
    on_intel do
      url "${BASE}/discodrive-daemon-darwin-amd64.tar.gz"
      sha256 "${SHA_DARWIN_AMD64}"
    end
  end

  on_linux do
    on_arm do
      url "${BASE}/discodrive-daemon-linux-arm64.tar.gz"
      sha256 "${SHA_LINUX_ARM64}"
    end
    on_intel do
      url "${BASE}/discodrive-daemon-linux-amd64.tar.gz"
      sha256 "${SHA_LINUX_AMD64}"
    end
  end

  def install
    bin.install "discodrive"
  end

  def caveats
    <<~EOS
      Pair with your DiscoDrive server first:
        discodrive pair --server https://your-server.example
      Then run it in the foreground with \`discodrive run\`, or install it
      as a login service with \`discodrive install\`.
    EOS
  end

  test do
    output = shell_output("#{bin}/discodrive 2>&1", 2)
    assert_match "pair|run|tray|status|install|uninstall", output
  end
end
EOF
